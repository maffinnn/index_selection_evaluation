...
class PostgresDatabaseConnector(DatabaseConnector):
    ...
    def get_plan_with_statistics(self, query):
        query_text = self._prepare_query(query)
        statement = f"explain (analyze, buffers, format json) {query_text}"
        query_plan = self.exec_fetch(statement)[0][0]["Plan"]
        self._cleanup_query(query)
    ...
    def table_row_count(self, table_name):
        statement = f"""SELECT reltuples FROM pg_class WHERE relname = '{table_name}';"""
        result = self.exec_fetch(statement)
        return result[0]
    
    def get_column_cardinality(self, column):
        column_name = column.name
        table_name = column.table.name
        statement = f"""SELECT n_distinct FROM pg_stats WHERE tablename='{table_name}' AND attname='{column_name}';"""
        result = self.exec_fetch(statement)
        return result[0]
    
    def count_clustered_indexes(self, table_name):
        statement = f"""SELECT COUNT(idx)
            FROM pg_index AS idx JOIN pg_class AS i ON i.oid = idx.indexrelid
            WHERE idx.indisclustered AND idx.indrelid::regclass = '{table_name}'::regclass;"""
        result = self.exec_fetch(statement)
        return result[0]
    
    def count_indexes(self, table_name):
        statement = f"""SELECT COUNT(idx)
            FROM pg_index AS idx JOIN pg_class AS c ON c.oid = idx.indexrelid
