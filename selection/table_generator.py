...
class TableGenerator:
    ...
    def _get_table_statistics(self, database_connector):
        for table in self.tables:
            row_count = database_connector.table_row_count(table.name)
            table.set_row_count(row_count)
            for column in enumerate(table.columns):
                card = database_connector.get_column_cardinality(column)
